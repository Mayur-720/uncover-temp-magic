...
import TagSuggestion from "@/components/tags/TagSuggestion";
...
const CreatePostModal = ({
	...
	const [mediaFiles, setMediaFiles] = useState<
		Array<{ type: "image" | "video"; url: string; file?: File }>
	>([]);
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			const extractedHashtags = contentHashtags.map((tag) =>
				tag.slice(1).toLowerCase()
			);

			// Combine selected tags with extracted hashtags
			const allTags = [...new Set([...selectedTags, ...extractedHashtags])];
			...
			<DialogContent className="sm:max-w-[600px] bg-background border-border max-h-[90vh] overflow-y-auto">
				...
				<form onSubmit={handleSubmit} className="space-y-4">
					<div className="flex items-center space-x-3 p-3 bg-muted/50 rounded-lg">
						...
						<div>
							...
							<p className="text-sm text-muted-foreground">
								{ghostCircleId
									? "Posting to Ghost Circle"
									: `Posting to ${getFeedDisplayName()} feed`}
							</p>
						</div>
					</div>
					...
					<Textarea
						...
						placeholder="What's on your mind? Share your thoughts anonymously... Use #hashtags to categorize your post!"
